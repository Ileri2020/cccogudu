...
export default function Similar(props: {similar : any}) {
  ...
  const cart = (name : string, id : string, img : any) => {
    //download file
            ...
            {/* <div className="flex flex-col text-nowrap items-center max-w-[240px] justify-between">
              ...
              <div className="flex flex-1 flex-col text-center justify-center items-center w-full /bg-red-500">
                  ...
                  <div className="flex flex-row w-full max-w-[100px] gap-1">
                      ...
                      <Link href={`/preach/${file._id}`}><Button variant={"outline"} className="rounded-lg flex-1 w-full h-6 font-semibold text-accent-secondary border-accent-secondary hover:bg-accent-secondary/60 hover:text-background border-2">play icon</Button></Link>
                      <Button onClick={() => cart(file.name, file._id, file.img)} variant={"outline"} className="rounded-lg flex-1 w-full h-6 font-semibold text-accent-secondary border-accent-secondary hover:bg-accent-secondary/60 hover:text-background border-2"><BiDownload /></Button>
                  </div>
              </div>
            </CarouselItem>
          )
